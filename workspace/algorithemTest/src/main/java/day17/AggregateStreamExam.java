...
public class AggregateStreamExam {
    public static void main(String[] args) {
        ...
        int max = Arrays.stream(intArr).boxed().max(Integer::compareTo).orElse(0); //orElse->비엇으면 0을 출력->optional값 0
        System.out.println(max);
        System.out.println("================================");

        List<Integer> numbers = Arrays.asList(1,5,3,88,2);
        long count = numbers.stream().count(); // 요소 개수
        int maxVal = numbers.stream().max(Integer::compareTo).orElse(0); // 최대 값, List가 비어있으면 0 반환
